...
int searchForInitializaion(Frame& refFrame, Frame& currFrame, int th, vector<int>& vMatches, std::vector<cv::Point2f>& vPrevMatched){
    ...
    for(size_t i = 0; i < vRefKeys.size(); i++){
        ...
        currFrame.getFeaturesInArea(vRefKeys[i].pt.x,vRefKeys[i].pt.y,15,minOctave,maxOctave,vIndicesToCheck);
...
int searchWithProjection(Frame& currFrame, int th, std::vector<std::shared_ptr<MapPoint>>& vMapPoints){
    ...
    for(shared_ptr<MapPoint> pMP : vMapPoints){
        ...
        // Check if the MapPoint's viewing direction aligns roughly with the camera 
        assert(pMP);
        ...
        if(viewCos < 0.5){
            vCos++; // Counter for points with poor viewing angle
            continue;
        }

        //Check distance is in the scale invariance region of the MapPoint
        // Check if the MapPoint's distance is within its scale invariant region
        ...
        if(dist < minDistance || dist > maxDistance){
            invDist++; // Counter for points outside scale invariant range
            continue;
        }

        //Predict scale
        // Estimate the appropriate image level (octave) for the MapPoint
        ...
        /*
         ...
         - Retrieve candidate features from the frame within the search radius
         - Match the MapPoint's descriptor with the candidate features
         */

        currFrame.getFeaturesInArea(uv.x, uv.y, radius, 
                                    predictedOctave - 1, predictedOctave + 1, vIndicesToCheck);


        // Perform feature matching using Hamming distance
        int bestDist = 255, secondBestDist = 255;
        int bestIdx;
        cv::Mat mapPointDescriptor = pMP->getDescriptor();

        for (size_t idx : vIndicesToCheck) {
            // Skip features already associated with map points.
            if (currFrame.getMapPoint(idx)) 
                continue; 

            int dist = HammingDistance(mapPointDescriptor.row(0), currFrame.getDescriptors().row(idx));

            // Update best and second best matches
            if (dist < bestDist && dist < th) {
                secondBestDist = bestDist;
                bestDist = dist;
                bestIdx = idx;
            }
        ...
        // If a strong match is found, associate the feature with the MapPoint
        if (bestDist < th && (float)bestDist < (float)secondBestDist * 0.9) { 
