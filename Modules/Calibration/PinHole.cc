...
#define fx vParameters_[0] // fx is 
...
void PinHole::projectJac(const Eigen::Vector3f& p3D, Eigen::Matrix<float,2,3>& Jac) {
    const float inv_z = 1.0f / p3D[2];
    const float inv_z_sq = inv_z * inv_z;

    // Partial derivatives with respect to x, y, and z components of the 3D point
    Jac(0, 0) = fx * inv_z;
    Jac(0, 1) = 0.0f;
    Jac(0, 2) = -fx * p3D[0] * inv_z_sq;

    Jac(1, 0) = 0.0f;
    Jac(1, 1) = fy * inv_z;
    Jac(1, 2) = -fy * p3D[1] * inv_z_sq;
}
